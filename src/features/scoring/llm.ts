...
export type ListDef = { slug: string; name: string; description?: string | null };
...
function validateAndRepair(r: unknown, validSlugs: Set<string>): ScoreResponse | false {
  const out: ScoreItem[] = [];
  ...
  for (const it of items) {
    ...
    const why = typeof it?.why === "string" ? it.why.trim().replace(/\s+/g, " ") : undefined;
...
export async function scoreRepoAgainstLists(
  ...
  const userPrompt = `Your task is to score the repository against EACH list from 0 to 1. where 1 = perfect fit. Multiple lists may apply. Provide a reason why for repos that meet the criteria. 
