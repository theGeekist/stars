...
import type { BatchSelector, PlanResult, ResumeFlag, ScoringService, Thresholds } from "./types";

// Prepared queries mirror current cli-scorer selection with resume filtering
type BindRunLimit = [runIdNullCheck: number | null, runIdForExists: number | null, limit: number];
type BindSlugRunLimit = [slug: string, runIdNullCheck: number | null, runIdForExists: number | null, limit: number];
...
let iScore = db.query<unknown, [number, number, string, number, string | null]>(`
...
export function createScoringService(): ScoringService {
  function getLastRunId(): number | null {
    const row = db.query<{ id: number | null }, []>(`SELECT MAX(id) AS id FROM model_run`).get();
    return row?.id ?? null;
  }
  ...
  function resolveRunContext(opts: { dry: boolean; notes?: string; resume?: ResumeFlag }): { runId: number | null; filterRunId: number | null } {
    if (opts.resume === "last") {
    ...
    if (typeof opts.resume === "number") {
      const ok = db.query<{ ok: number }, [number]>(`SELECT 1 ok FROM model_run WHERE id = ?`).get(opts.resume);
      if (!ok) throw new Error(`--resume ${opts.resume} does not exist`);
      return opts.dry ? { runId: null, filterRunId: opts.resume } : { runId: opts.resume, filterRunId: opts.resume };
    }
  ...
  function selectRepos(sel: BatchSelector, filterRunId: number | null): RepoRow[] {
    const limit = Math.max(1, Number(sel.limit ?? 10));
    if (sel.listSlug) return qBatchBySlug.all(sel.listSlug, filterRunId, filterRunId, limit);
    return qBatchDefault.all(filterRunId, filterRunId, limit);
  }

  function persistScores(runId: number, repoId: number, scores: ScoreItem[]): void {
    for (const s of scores) iScore.run(runId, repoId, s.list, s.score, s.why ?? null);
  }

  function planTargets(current: string[], scores: ScoreItem[], cfg?: Thresholds): PlanResult {
    ...
    const scoreMap = new Map<string, number>(scores.map((s) => [s.list, s.score]));
    const keep = current.filter((slug) => (scoreMap.get(slug) ?? 0) > removeTh);

    const review = scores
      .filter((s) => s.score > removeTh && s.score < (addBySlug[s.list] ?? defaultAdd))
      .map((s) => s.list);
    ...
    const remove = current.filter((s) => !planned.includes(s) && !preserve.has(s));

    return { planned, add, remove, keep, review };
  }

  return { getLastRunId, createRun, resolveRunContext, selectRepos, persistScores, planTargets };
}
