...
export function createListsService(): ListsService {
  ...
  async function reconcileLocal(repoId: number, slugs: string[]): Promise<void> {
  ...
  async function ensureListGhIds(token: string): Promise<Map<string, string>> {
    ...
    const rows = db
      .query<{ id: number; slug: string; name: string; list_id: string | null }, []>(
  ...
  async function ensureRepoGhId(token: string, repoId: number): Promise<string> {
    const r = db
    ...
    if ((r as any).repo_id && /^R_kg/.test((r as any).repo_id)) return (r as any).repo_id as string;
    ...
    db.query<unknown, [string, number]>(`UPDATE repo SET repo_id = ? WHERE id = ?`).run(
      newId,
      repoId,
    );
    return newId;
