...
function shouldRetry(status: number) { return status >= 500 || status === 429 || status === 403; }
...
export async function githubGraphQL<T>(
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    try {
      DEBUG && console.error(`[gql] POST attempt ${attempt + 1}/${MAX_RETRIES} timeout=${DEFAULT_TIMEOUT_MS}ms vars=${JSON.stringify(variables ?? {})}`);
      ...
      if (shouldRetry(res.status) && attempt < MAX_RETRIES - 1) {
        ...
        DEBUG && console.error(`[gql] attempt ${attempt + 1} status=${res.status} backoff=${backoff}ms`);
        await sleep(backoff);
        continue;
      }
      ...
      clearTimeout(timer);
      if (attempt === MAX_RETRIES - 1) throw err instanceof Error ? err : new Error(String(err));
    }
  }
