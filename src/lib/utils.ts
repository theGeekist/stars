...
export function linkDensity(s: string): number {
  ...
  const linkish = lines.filter((l) => /\[[^\]]+\]\([^)]+\)|https?:\/\//i.test(l)).length;
...
export function isAwesomeList(
  ...
  const metaHit =
    ...
    tops.some((t) =>
      ["awesome", "awesome-list", "awesome-lists", "awesome-collection"].includes(t)
    );
...
export function summariseAwesomeList(
  ...
  const cats = (topics || []).filter((t) => !/awesome/i.test(t)).slice(0, 4).join(", ");
  const scopeHint = cats || (description ? description.replace(/\s+/g, " ").trim() : "");
  const base =
